...
def test(args):
    ...
    for i, batch in enumerate(data_loader):
        ...
        for j in range(output_points.shape[0]):  # iterate through the batch
            ...
            if cfg["dataset"] == "nuscenes":
                fname = filenames[j]
                scene, sample, sd = fname[0], fname[1], fname[2]
            else:
                fname = filenames[j]
                scene, sample = fname[0], fname[1]

            if args.assume_constant_velocity:
                displacement = fname[-1]
                output_origin = torch.zeros_like(output_origin)
                displacements = torch.arange(_n_output) + 1
                output_origin = (output_origin + displacements[None, :, None]) * displacement[None, None, :]
