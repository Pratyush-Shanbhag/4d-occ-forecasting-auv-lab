...
def make_data_loaders(args):
    ...
    elif args.dataset.lower() == "marine":
        from data.marine import MarineDataset

        data_loaders = {
            "train": DataLoader(
                MarineDataset(args.marine_root, args.marine_cfg, "trainval", dataset_kwargs),
                collate_fn=CollateFn,
                **data_loader_kwargs,
            ),
            "val": DataLoader(
                MarineDataset(args.marine_root, args.marine_cfg, "test", dataset_kwargs),
                collate_fn=CollateFn,
                **data_loader_kwargs,
            ),
        }
...
if __name__ == "__main__":
    ...
    data_group.add_argument("--marine-root", type=str, default=None)
    data_group.add_argument("--marine-cfg", type=str, default="configs/marine.yaml")
    data_group.add_argument(
        "--pc-range",
        type=float,
        nargs="+",
